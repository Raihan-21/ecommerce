...
const modifyQty = async (userid, itemid, inc, price) => {
  const takeItem = await Item.findOneAndUpdate(
    { _id: itemid },
    { $inc: { quantity: -inc } }
  );
  const add = await Cart.findOneAndUpdate(
    { userid, "items._id": mongoose.Types.ObjectId(itemid) },
    {
      $inc: {
        "items.$.quantity": inc,
...
module.exports.updateuser = async (req, res) => {
  const { userid, fullname, email, city, kecamatan, kelurahan, address } =
    req.body;
  const updateInfo = await Customer.findOneAndUpdate(
    { _id: userid },
    {
      $set: {
        email,
...
module.exports.updateitem = async (req, res) => {
  ...
  const item = await Item.findOneAndUpdate(
    { _id: itemid },
    { itemname, category, price, desc, quantity }
  );
...
module.exports.catalogue = async (req, res) => {
  ...
  if (category) {
    ...
    const items = await Item.find({ itemname: new RegExp(itemname, "i") }).sort(
      { itemname: 1 }
    );
    res.render("catalogue", { items, category });
  }
...
module.exports.addCart = async (req, res) => {
  ...
  const addItem = await Cart.updateOne(
    { userid: user._id },
    { $push: { items: cartItem }, $inc: { totalprice, totalquantity: qty } },
    { upsert: true }
  );
...
module.exports.removeItem = async (req, res) => {
  ...
  const item = await Cart.findOne(
    { userid: user._id },
    { items: { $elemMatch: { _id: mongoose.Types.ObjectId(itemid) } } }
  );
  const storeItem = await Item.findOneAndUpdate(
    { _id: itemid },
    { $inc: { quantity: qty } }
  );
  const remove = await Cart.findOneAndUpdate(
    { userid: user._id },
    {
      ...
      $inc: {
        totalquantity: -item.items[0].quantity,
...
module.exports.checkout = async (req, res) => {
  ...
  //     receipt
  // }})
  const checkout = await Cart.findOneAndUpdate(
    { userid: user._id },
